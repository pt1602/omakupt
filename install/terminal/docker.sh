...
if [ ! -f /etc/apt/sources.list.d/docker.list ]; then
    ...
    DOCKER_SUITE=$(repo_best_suite https://download.docker.com/linux/ubuntu) || {
        omakupt_note "Docker has no repo for $(omakupt_ubuntu_codename) or any earlier release" >&2
        return 1 2>/dev/null || exit 1
    }

    if [ "$DOCKER_SUITE" != "$(omakupt_ubuntu_codename)" ]; then
        omakupt_note "Docker has no $(omakupt_ubuntu_codename) repo yet, using $DOCKER_SUITE"
    fi
