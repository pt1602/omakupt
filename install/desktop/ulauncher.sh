...
if ! add_ppa_if_published ppa:agornostal/ulauncher ulauncher; then
	omakupt_note "skipping Ulauncher; rerun 'omakupt install' once the PPA catches up"
...
cp ~/.local/share/omakupt/configs/ulauncher.desktop ~/.config/autostart/ulauncher.desktop
gtk-launch ulauncher.desktop >/dev/null 2>&1
sleep 2 # ensure enough time for ulauncher to set defaults
cp ~/.local/share/omakupt/configs/ulauncher.json ~/.config/ulauncher/settings.json
