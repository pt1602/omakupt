...
# Compatibility helpers for running Omakupt across Ubuntu releases.
#
...
# that were correct for one release is what breaks Omakupt on the next one, so the
# installers ask these helpers what is actually available on the running system.

[ -n "${OMAKUPT_COMPAT_LOADED:-}" ] && return 0
OMAKUPT_COMPAT_LOADED=1

omakupt_note() { echo "Omakupt: $*"; }
...
omakupt_ubuntu_codename() { omakupt_os_release UBUNTU_CODENAME; }
omakupt_ubuntu_version() { omakupt_os_release VERSION_ID; }
...
omakupt_older_codenames() {
	local csv=/usr/share/distro-info/ubuntu.csv
	[ -f "$csv" ] || return 0

	awk -F, -v current="$(omakupt_ubuntu_version)" '
		NR > 1 {
			split($1, release, " ")
...
apt_install_first() {
	...
	if package=$(apt_first_available "$@"); then
		...
		omakupt_note "none of these packages exist on $(omakupt_ubuntu_codename): $*" >&2
		return 1
	fi
}
...
repo_best_suite() {
	...
	for suite in "$(omakupt_ubuntu_codename)" $(omakupt_older_codenames); do
...
ppa_publishes_package() {
	local ppa=${1#ppa:} package=$2 series=${3:-$(omakupt_ubuntu_codename)}
...
add_ppa_if_published() {
	...
	omakupt_note "$ppa has no $package build for $(omakupt_ubuntu_codename)"
	return 1
}
...
gsettings_set_if_available() {
	...
	if gsettings list-keys "${schema%%:*}" 2>/dev/null | grep -qx -- "$key"; then
		...
		omakupt_note "skipping unavailable setting ${schema%%:*} $key"
	fi
